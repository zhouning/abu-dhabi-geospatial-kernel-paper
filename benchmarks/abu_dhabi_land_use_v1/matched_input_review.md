...
All runs used the supplied macOS arm64 binary, seed 31, the 2021 training year,
the same 2022 driver year and the same CA protocol as the headline control.
...
| `matched_kernel` | 25 (6 one-hot + 12 neighbourhood + 7 continuous, Kernel order) | 0.1320 | 0.1960 | `artifacts/predictions/flus_matched_inputs_abs/report.json` |

The values are single-seed diagnostics and are not substituted for the
three-seed headline comparison. The full command pattern is:
...
25-feature run returned code 0, wrote `target_probability.tif`, and produced
valid 2023 and 2024 rasters. The archive therefore supports the narrower
conclusion that the previous failure was a path/configuration problem, while
the primary paper comparison remains an unmatched seven-driver control versus
the 25-feature Geospatial Kernel pipeline.
