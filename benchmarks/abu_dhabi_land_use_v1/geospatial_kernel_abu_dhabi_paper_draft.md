...
Geospatial Kernel improved change FoM over GeoSOS-FLUS by 0.0721 in 2023 and 0.0790 in 2024. Paper58 improved over GeoSOS-FLUS by 0.0388 and 0.0963, respectively. The ranking changed at the longer horizon: Geospatial Kernel was strongest for the 2023 one-step target, while Paper58 was strongest for the 2024 two-step target. The result is scientifically useful precisely because it prevents an over-general conclusion. Geospatial Kernel was competitive in historical allocation, but its advantage in this benchmark is not a universal claim about future prediction accuracy.
...
| Paper58 | Compact | 0.00000 | 0.00% | 0.7676 ± 0.0170 | 475.9 ± 35.7 | 239.7 ± 44.6 | 3,500 | 0 | 350 | no |
| Paper58 | Ecological priority | 0.00000 | 0.00% | 0.7506 ± 0.0174 | 452.1 ± 22.0 | 222.3 ± 19.8 | 2,450 | 0 | 1,050 | no |
| Paper58 | Outward growth | 0.00000 | 0.00% | 0.8117 ± 0.0236 | 507.5 ± 56.4 | 263.0 ± 71.8 | 7,000 | 0 | 175 | no |
...
The built-retirement contrast is also visible in the maps. GeoSOS-FLUS reclassified approximately 2,724–3,632 cells that were built in 2024 while allocating new built cells elsewhere. Geospatial Kernel and Paper58 retained the prior built class under the same scenario target counts. This difference is a model behaviour under the frozen transition matrix, not evidence that either approach reproduces actual redevelopment or demolition.
...
**Figure 4 | Spatial allocations and change footprints.** Three rows show compact, ecological-priority and outward-growth actions; columns show GeoSOS-FLUS, Geospatial Kernel and Paper58. Colours distinguish retained water/wetland, retained vegetation, new built cells, retired built cells and bare/other cells. The map is an ensemble or majority raster at 2030 in the current draft. Draft asset: `planning_2030_comparison.png`. The public vector packages contain annual and cumulative transition layers for 2027–2031.
...
## Author-facing editorial notes (not part of the submission manuscript)

### Assumptions or missing inputs

- The paper uses the public benchmark state as land cover. It does not claim to recover Abu Dhabi's legal land-use categories.
- No official 2025–2031 observations are available. All future results are scenario stress tests driven by documented target counts.
- The current figures are draft evidence assets. Figure labels, legends and typography must be translated and regenerated in English before submission.
- The required action-deleted, action-shuffled, constraint-deleted, spatial-driver-shuffled, persistence and Markov/CA controls are not yet part of the paper's evidence base.

### Claim–evidence map

| Claim | Evidence | Status |
|---|---|---|
| The Kernel exposes a state–action–constraint execution contract | `runtime.py`, `run_geospatial_kernel.py`, per-step audit records | supported |
| Kernel has the best historical performance at every horizon | 2023 and 2024 comparison metrics | not supported; explicitly rejected |
| Kernel is competitive in historical conditional allocation | Table 1 and `comparison_report.json` | supported, horizon-dependent |
| Kernel gives feasible and more compact/road-accessible 2031 allocations | Table 2, planning report and output audit | supported under frozen public-data objectives |
| The three future trajectories are official Abu Dhabi predictions | No official actions or future labels | unsupported; excluded |
| Neighbourhood context caused the planning advantage | Limited weight sensitivity only; controls pending | needs evidence |
| Output polygons are cadastral parcels | Raster-cell dissolve only | unsupported; excluded |

### Why this structure

- The paper introduces the planning problem before the implementation details, then separates the Kernel design from its empirical evaluation.
- Historical validation is reported before future scenarios so readers see both the strongest result and the horizon where another model wins.
- Planning objectives are reported component by component and on a conditional Pareto frontier rather than through an opaque weighted score.
- Data semantics and unsupported uses are repeated at the point where a reader might otherwise interpret land cover as legal land use.

### Submission-critical revisions

1. Add the protocol-required negative controls or narrow all mechanism wording to design intent.
