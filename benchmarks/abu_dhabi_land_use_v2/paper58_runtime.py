...
def build_demand_conditioned_model(
    n_classes: int,
    *,
    z_dim: int = Z_DIM,
    n_context: int = N_CONTEXT,
) -> LatentDynamicsModule:
    """Build an LDN whose conditioning input is a land-demand action vector."""

    if int(n_classes) <= 0:
        raise ValueError("n_classes must be positive")
    return _build_model(
        z_dim=z_dim,
        scenario_dim=2 * int(n_classes),
        n_context=n_context,
    )
