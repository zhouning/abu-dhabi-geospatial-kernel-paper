...
class FlusInputs:
    ...
    def features(self, year: int, *, include_road_snapshot: bool = True) -> np.ndarray:
        layers = [
            self.x[self.valid],
            self.y[self.valid],
        ...
        return np.column_stack(layers).astype(np.float32)

    def matched_kernel_features(self, year: int, *, include_road_snapshot: bool = True) -> np.ndarray:
    ...
    def feature_bundle(
        ...
        *,
        ...
        """
        baseline = self.features(year, include_road_snapshot=include_road_snapshot)
        baseline_names = list(BASELINE_FEATURE_NAMES if include_road_snapshot else BASELINE_FEATURE_NAMES[:5])
        state = self.states[year]
        one_hot = np.stack([(state == value).astype(np.float32) for value in CLASSES])
        valid_float = self.valid.astype(np.float32)
        neighbourhoods = []
        ...
        if feature_mode == "matched_kernel":
            ...
            if include_road_snapshot:
                continuous_names.extend(["log_distance_road_norm", "log_distance_major_road_norm"])
            return self.matched_kernel_features(year, include_road_snapshot=include_road_snapshot), [
                *(f"current_class_{value}" for value in CLASSES),
                *neighbourhood_names,
                *continuous_names,
            ]
...
def train_suitability(
    ...
    *,
    ...
    fit_years: tuple[int, ...] = FIT_YEARS,
    include_road_snapshot: bool = True,
) -> tuple[np.ndarray, dict[str, Any]]:
    if not fit_years:
        raise ValueError("flus_fit_years_empty")
    training_bundles = [
    ...
    target_features, target_feature_names = inputs.feature_bundle(
        target_driver_year,
        feature_mode,
        include_road_snapshot=include_road_snapshot,
    )
    ...
    return cube, {
        ...
        "road_snapshot_features_used": include_road_snapshot,
...
def simulate_year(
    ...
    *,
    ...
    hard = inputs.hard if hard_exclusion_mask is None else np.asarray(hard_exclusion_mask, dtype=bool)
    if valid.shape != current.shape or hard.shape != current.shape:
        raise ValueError("flus_simulation_mask_shape_mismatch")
    allowed = (valid & ~hard).astype(np.uint8)
    ...
    result[~valid] = 0
    if np.any(result[valid & hard] != current[valid & hard]):
