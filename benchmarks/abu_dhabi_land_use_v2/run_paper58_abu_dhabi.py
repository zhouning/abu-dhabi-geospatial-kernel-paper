...
FIT_TRANSITIONS = ((2017, 2018), (2018, 2019), (2019, 2020), (2020, 2021), (2021, 2022), (2022, 2023), (2023, 2024))
VALIDATION_TRANSITION = (2024, 2025)
...
class BenchmarkData:
    ...
    def training_patches(
        self,
        transitions: tuple[tuple[int, int], ...] = FIT_TRANSITIONS,
    ) -> list[tuple[int, int, int, int]]:
        patches = []
        for start_year, target_year in transitions:
...
def train_decoder(
    ...
    *,
    ...
    label_years: tuple[int, ...] = tuple(range(2017, 2025)),
    validation_year: int | None = VALIDATION_TRANSITION[1],
) -> tuple[LogisticRegression, dict[str, Any]]:
    if not label_years:
        raise ValueError("decoder_label_years_empty")
    ...
    report = {
        ...
        "label_years": list(label_years),
    }
    if validation_year is not None:
        validation_embedding = data.embedding(validation_year).reshape(64, -1)[:, data.valid.ravel()].T
        validation_label = data.states[validation_year][data.valid] - 1
        report.update(
            validation_accuracy_year=int(validation_year),
            validation_accuracy=float(decoder.score(validation_embedding, validation_label)),
        )
    else:
        report.update(validation_accuracy_year=None, validation_accuracy=None)
...
def train_ldn(
    ...
    *,
    ...
    learning_rate: float,
    device: torch.device,
    fit_transitions: tuple[tuple[int, int], ...] = FIT_TRANSITIONS,
    validation_transition: tuple[int, int] = VALIDATION_TRANSITION,
) -> tuple[torch.nn.Module, dict[str, Any]]:
    if not fit_transitions:
        raise ValueError("ldn_fit_transitions_empty")
    ...
    patches = data.training_patches(fit_transitions)
    rng = np.random.default_rng(seed)
    ...
    for epoch in range(epochs):
        ...
        validation = validate_ldn(
            ...
            device=device,
            transition=validation_transition,
        )
    ...
    return model, {
        ...
        "validation_transition": list(validation_transition),
        "epochs": epochs,
...
def validate_ldn(
    ...
    *,
    ...
    decoder_bias: torch.Tensor,
    device: torch.device,
    transition: tuple[int, int] = VALIDATION_TRANSITION,
) -> dict[str, float]:
    start_year, target_year = transition
