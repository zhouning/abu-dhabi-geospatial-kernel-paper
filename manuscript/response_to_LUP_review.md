...
| R9.3 | Reviewer-provided x86_64 values should be replaced by an author-controlled repeat where feasible | Added a GitHub Actions Ubuntu x86_64 regression workflow. Run `34107504198` on commit `e06a997` installed the resolved Python 3.11 lock, re-ran the three Kernel seeds, and matched all six released categorical rasters with 0 differing pixels. Its JSON audit is now archived in the bundle and the workflow uploads each future audit. | The existing Windows FLUS values remain accurately labelled as reviewer-provided external verification because their rasters are not locally archived. The workflow is a persistent author-controlled check for the Kernel only. |
...
| External product diagnostic | Materialized and hashed ESA WorldCover 2020 v1.0 and 2021 v2.0 built-fraction rasters at the benchmark grid; compared built stock and built gain across thresholds 0.05–0.30. | The former Dynamic World-count built-gain result is now identified as structural-zero and zero-powered because built count declines. The new WorldCover-count action gives Kernel F1 0.0157–0.0397 versus random 0.0251–0.0277; after adding three-seed SDs, Kernel is below random at 0.05, indistinguishable at 0.10–0.20, and clearly above random only at 0.30. Dynamic World-to-WorldCover gain F1 remains 0.0111–0.0192 as evidence of product disagreement. |
| Reproducibility closure | Added tests, synchronized Supplementary Table S5 and Fig. 3, updated manuscript consistency assertions, regenerated all outputs and reran the complete reproduction entry point. | The audit now covers 312 historical, rolling-origin and planning prediction rasters plus two frozen WorldCover evidence rasters with zero failures; all are included in generated-output hashes. |
...
## Fourteenth-round minor revision (8 September 2026)

| Reviewer point | Revision | Evidence boundary |
|---|---|---|
| Dynamic World-count built-gain F1 was a zero-power test | Rewrote the Abstract, Highlights, Results and Discussion to state that the 2020→2021 built-count decline makes built gain impossible under oracle demand. The zero F1 is no longer interpreted as model failure. | Only Dynamic World-to-WorldCover gain agreement remains informative about label-product disagreement in that protocol. |
| Informative external allocation test was missing | Added a WorldCover-count action: for every threshold, the 2020-origin, road-free Kernel proposal ranks mutable non-built cells; random allocation uses the same candidates and count. | Kernel mean ± SD is 0.0157 ± 0.0008, 0.0228 ± 0.0007, 0.0268 ± 0.0011 and 0.0397 ± 0.0007; random is 0.0268 ± 0.0058, 0.0277 ± 0.0036, 0.0251 ± 0.0011 and 0.0251 ± 0.0075. Kernel is below random at 0.05, indistinguishable at 0.10–0.20 and clearly above only at 0.30. This mixed result is product agreement, not authoritative validation. |
| Stock baselines were incomplete | Added Dynamic World 2020 and 2021 stock curves and table columns. | Their F1 ranges, 0.3908–0.4334 and 0.3501–0.3924, show that persistence's apparent advantage mirrors product-year agreement. |
| Headline road snapshot leaks future information | Methods and Limitations now identify the 31 July 2026 OSM snapshot and distinguish symmetric ranking effects from potentially biased absolute scores. | The 23-feature rolling-origin experiment is the strict no-road-snapshot evidence. |
| New rasters were outside audit and hash coverage | Added all 36 rolling prediction rasters to the class, grid and constraint audit and added those rasters plus two frozen WorldCover rasters to generated-output hashes. | The current audit covers 312 predictions and two evidence rasters with zero failures. |
| Fig. 3 and Table S5 needed repair | Moved legends into unused panel space, expanded the built-gain axis, added both Dynamic World stock curves, replaced the zero-powered panel with the WorldCover-count action, and added non-comparability notes for rolling versus headline estimates. | Figure source data are regenerated directly from the versioned reports. |
| The 2022 rolling low point lacked context | Added the synchronized fall in Kernel and random scores and linked it to Fig. S2 label reversion and built-count fluctuation. | The wording does not attribute the difficult transition to a Kernel-specific defect. |

## Fifteenth-round proof-stage revision (8 September 2026)

| Reviewer point | Revision | Evidence boundary |
|---|---|---|
| WorldCover-count action reported means without dispersion | Added the three-seed sample SD for both the Kernel allocation and the matched random allocation at all four WorldCover thresholds. The values are now present in the diagnostic JSON, Fig. 3d source data and Supplementary Table S5. | The SD describes variation across the three declared seeds; it is not an uncertainty interval for authoritative Abu Dhabi truth. |
| The conclusion at the two larger thresholds was too strong | Replaced “Kernel higher at the two larger thresholds” with the evidence-calibrated statement that the 0.05 threshold is below random, 0.10–0.20 cannot be distinguished from random within three-seed variation, and only 0.30 is clearly above random. The Abstract, Results, Discussion and Supplementary Table S5 now use the same wording. | This is an external-product agreement diagnostic, not a validated land-use forecast or a claim of general model superiority. |
| The low 0.05 result required a mechanism | Added the product-definition explanation that pixels just above 5% WorldCover built fraction are commonly sparse or road-edge structures; Dynamic World does not necessarily label them as built. A Kernel trained on Dynamic World transitions can therefore rank those WorldCover-only low-density pixels late, producing below-chance agreement at this threshold. | The explanation is conditional on the two products’ class definitions and spatial resolution; it does not assert that the WorldCover pixels are erroneous. |
| DOI status was stale in the review record | The submission package now cites the archival reproducibility DOI `10.5281/zenodo.22663476` (version DOI; concept DOI `10.5281/zenodo.22663475`) and the GitHub tag `v1.0.0`. | This DOI identifies the public reproducibility package, not a future *Landscape and Urban Planning* article DOI and not an authoritative Abu Dhabi data release. |

The fifteenth-round changes require no new model fitting or raster generation beyond
the already completed WorldCover diagnostic rerun. The final release was rebuilt
