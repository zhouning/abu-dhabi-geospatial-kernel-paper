...
| Matched-input FLUS was not attempted | The earlier failure is now identified as a relative-path configuration error (`read config file error`), not a verified segmentation fault. With absolute paths, the supplied Mach-O arm64 console completed the 25-feature matched-input run for seed 31 and wrote a valid probability surface plus 2023/2024 predictions. Two intermediate configurations (13 features: seven drivers plus current-class indicators; 19 features: seven drivers plus neighbourhood fractions) also completed for seed 31. Commands, configurations, logs and reports are archived under the corresponding `artifacts/predictions/` directories. | The headline comparison remains the original three-seed seven-driver FLUS-style control versus the 25-feature Kernel; the matched and intermediate runs are one-seed dimensionality diagnostics, not replacement baselines. |
...
2. The FLUS-style binary is a macOS arm64 executable with no recoverable source
   ...
   the successful 25-feature seed-31 run is retained as a dimensionality
   diagnostic and does not change the primary comparison.
