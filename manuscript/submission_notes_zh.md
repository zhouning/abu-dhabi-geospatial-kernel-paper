...
7. 投稿前仍需引入独立建成区产品或人工判读样本，验证 Dynamic World 2023–2024 的变化真实性；匹配输入 FLUS 已用绝对路径完成 13、19 特征的 seed-31 机制诊断及 25 特征的三种子诊断。25 特征运行仅 seed 31 有效，seeds 47、73 因同年类别身份泄漏退化为零变化，不能作为有效基线求均值。FLUS 基础源码可追溯至 GeoSOS 公开版本，论文使用的确定性修改源码已发布在 `FLUS_console_crossplatform` 标签 `paper-benchmark-flus-v1`。
