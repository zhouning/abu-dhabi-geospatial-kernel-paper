...
data and in Supplementary Table S4. In 2021, 2022 and 2024, 61.3–68.3% of
discordant cells were Dynamic World bare mapped as ArcGIS-served built (13,078,
18,787 and 17,987 cells, respectively); a further 3,723–4,496 cells per year
were Dynamic World bare mapped as ArcGIS-served water. Those transition counts
use the pairwise common-coverage grid. The ArcGIS-served product also mapped
4,501, 4,985 and 5,131 more water cells than Dynamic World in those three
years, using each product's own valid-cell total (whose denominator differs by
18–23 cells from the paired grid). Because origin-year water and wetland are
hard masks, this product difference reduces the feasible allocation domain in
the ArcGIS track. It is therefore a pipeline difference, not an isolated
replacement of target labels.
...
+0.205, -0.050 and -0.014 across the four matched years. The positive 2022
difference is the target that follows the documented ArcGIS series break. With
that target excluded, ArcGIS-minus-Dynamic World differences remain negative
in every retained fold for Kernel (-0.244 to -0.014) and GeoFM-LDN (-0.215 to
-0.010); FLUS-style ANN–CA remains mixed (-0.100 to +0.023). This is a
directional sensitivity of agreement with product-specific labels, not a claim
that either product is more accurate. Thus the learned allocator contains
repeatable spatial signal relative to its zero model, while its absolute score
and rank remain dependent on the product pipeline. Supplementary Table S6 gives
both interval definitions, and Supplementary Table S7 reports all seed-specific
paired intervals for Kernel versus FLUS-style and GeoFM-LDN versus Kernel. In
the ArcGIS 2023 target, all three GeoFM-LDN-minus-Kernel intervals include zero
despite Kernel's higher mean; in 2024, two exclude zero in Kernel's favour and
one includes zero. These are conditional intervals,
...
reported in Supplementary Table S7. Across all ArcGIS folds the bound spans
0.0858–0.8127, but excluding the 2022 series-break target it spans only
0.0858–0.6651. The reversal in these bounds means that
...
Replacing Dynamic World with a newer ArcGIS-served 10-m annual product changed
mapped built stock, transition prevalence and the leading model by year. The
only positive matched-product differences for the two exact-count learned
models occur in the 2022 target following the ArcGIS class-composition break;
outside that target, both Kernel and GeoFM-LDN have lower product-specific FoM
on every remaining paired fold. This is evidence of label-product dependence,
not evidence that either product is more accurate. Geospatial Kernel remained
consistently better than random minimum-change allocation, but it was not
universally the best learned model. Its strongest transferable property in this
experiment is therefore the auditable separation of learned proposal,
constrained projection and state writeback, not a product-independent accuracy
claim. The count-only bounds constrain only the exact minimum-change
projections; the FLUS baseline already shows that allowing more gross change
alone does not determine FoM. A controlled gross-transition comparison must
separate that budget from proposal-model skill.
...
The analysis scripts, vendored Geospatial Kernel runtime snapshot, benchmark protocol, model runners, figure-generation code and source-data tables are maintained in the accompanying repository. The benchmark READMEs provide the complete entry-point list; `reproducibility/reproduce.py`, `reproducibility/reproducibility_check.py` and `audit_outputs.py` are the end-to-end and fail-closed verification entry points. The cross-product workflow is documented beside `analyze_product_robustness.py`, with its frozen 2031 seed states and source data. This concise statement avoids duplicating the machine-readable inventory in the data manifests and supplementary files.
...
https://doi.org/10.5281/zenodo.22763326 and corresponds to GitHub tag `v2.1.0`
at commit `efb0961`; the DOI identifies the public benchmark source release,
...
- **Supplementary Table S4:** Annual six-class Dynamic World-to-ArcGIS product matrices on the paired grid (see `supplementary_table_S4_cross_product_class_matrices.md`).
...
- **Fig. S4:** Driver layers and unified experiment design. Continuous maps are clipped to the 2nd–98th valid-pixel percentiles unless stated otherwise. The schematic uses one coordinate system to keep arrows aligned and shows that the models change spatial allocation while the grid, actions, constraints and evaluator remain fixed (`figures/figS04_driver_layers_and_experiment_design.*`).
